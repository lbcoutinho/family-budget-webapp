...
import { type ApiErrorDto, type ErrorCode } from '../api-error';
...
export class PrismaExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(PrismaExceptionFilter.name);
    ...
    response.status(status).json({ statusCode: status, code, message } satisfies ApiErrorDto);
  }

  private map(exception: Prisma.PrismaClientKnownRequestError): { status: HttpStatus; code: ErrorCode; message: string } {
    switch (exception.code) {
      // Unique constraint failed.
      case 'P2002':
        return { status: HttpStatus.CONFLICT, code: 'DUPLICATE_NAME', message: 'A record with the same unique value already exists.' };
      ...
      case 'P2003':
        return { status: HttpStatus.CONFLICT, code: 'RECORD_IN_USE', message: 'Other records still reference this one. Deactivate it instead of deleting it.' };
      // Record required by the operation was not found.
      case 'P2025':
        return { status: HttpStatus.NOT_FOUND, code: 'RECORD_NOT_FOUND', message: 'The requested record was not found.' };
      default:
