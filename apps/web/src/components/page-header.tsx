...
export function PageHeader({ title, actions, className }: PageHeaderProps) {
  ...
  return (
    <header
      className={cn(
        'sticky top-0 z-20 flex flex-wrap items-center justify-between gap-3 border-b border-border bg-background/92 px-3.5 py-2.5 backdrop-blur-md shell:px-content shell:py-3',
        className,
      )}
    >
      <div className="flex min-w-0 items-center gap-2">
        ...
        <h1 className={cn('font-display text-headline font-bold tracking-headline', typeof title === 'string' ? 'truncate' : 'min-w-0')}>{title}</h1>
      </div>
...
export function PageContent({ children, className }: { children: ReactNode; className?: string }) {
  return <main className={cn('w-full max-w-page-content px-3.5 pt-4 pb-10 shell:p-content', className)}>{children}</main>;
}
