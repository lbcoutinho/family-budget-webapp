...
- GitHub Issues/Milestones are the source of truth for anything already ticketed
...
Impeccable is vendored into `.claude/` — the `impeccable` skill (`/impeccable audit`, `critique`, `polish`,
...
`Write`/`MultiEdit` on UI files and does a deeper pass on `Stop`. `npx impeccable detect <path>` runs the same detector standalone.
