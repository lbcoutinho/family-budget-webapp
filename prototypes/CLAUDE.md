...
- **State right now: `00-design-system.html`, `01-login.html`, `02-app-shell.html`, `06-month.html`, `09-reports-monthly.html`, `10-reports-yearly.html`,
  `11-reports-charts.html`, `12-recurrences.html` and `14-settings-general.html` are approved**
  and live in `approved/` (so M2-T06, M3-T06, the M5 month tickets, M7-T06 and M3-T13 are unblocked on the UI side);
  Every screen the project has drawn so far is approved; 13 is not drawn yet. v2 is direction **D — Cromático**, chosen from four candidates now in
